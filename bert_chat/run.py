...
def run(*argv):
	...
	args.name = time.strftime("run%Y%m%d_%H%M%S", time.localtime())
	args.restore = None
	...
	args.datapath = "./data/mytrain"
	args.epochs = 401
	args.wvclass = "FastText"
	args.wvpath = "/data/nlp/vec_model"
	args.bert_model = "/data/nlp/vec_model/pytorch"
	args.bert_vocab = "/data/nlp/vec_model/pytorch"
	args.out_dir = "./output"
	...
	args.decode_mode = "beam"
	args.top_k = 10
	args.length_penalty = 0.7
	...
	args.grad_clip = 5
	args.show_sample = [0]  # show which batch when evaluating at tensorboard
	args.max_sent_length = 35 # 文本最大长度
	args.checkpoint_steps = 40 # 每多长batch存一次
	args.checkpoint_max_to_keep = 100 # 最多存多少个模型
