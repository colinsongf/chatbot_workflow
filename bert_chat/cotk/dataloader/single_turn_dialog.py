...
class BERTOpenSubtitles(BERTSingleTurnDialog):
	...
	def __init__(self, file_id, min_vocab_times=10, \
		...
		self._min_vocab_times = 5
		print("i set min_vocab_times: ", self._min_vocab_times)
	...
	def _load_data(self):
		...
		def read_instances_from_file(sentences):
			sen_src = []
			sen_tgt = []
			for i in range(0, len(sentences) - 1, 2):
				sen_src.append(sentences[i])
				sen_tgt.append(sentences[i + 1])

			return [sen_src, sen_tgt]

		sentences = read_file("%s/xiaohuangji50w_nofenci.conv" % self._file_path)
		res = read_instances_from_file(sentences)

		post_tokens, post_bert_ids, resp_tokens, resp_bert_ids = self._mp_process(res[0], res[1])

		train = int(0.9 * len(post_tokens))
		dev = int(0.95 * len(post_tokens))

		print("total: ", len(post_tokens), "  train: ", train, "  dev: ", dev)

		for key in self.key_name:
			if key == "train":
				origin_data[key] = {}
				origin_data[key]['post'] = post_tokens[:train]
				origin_data[key]['resp'] = resp_tokens[:train]
				origin_data[key]['post_bert'] = post_bert_ids[:train]
				origin_data[key]['resp_bert'] = resp_bert_ids[:train]
			elif key == "dev":
				origin_data[key] = {}
				origin_data[key]['post'] = post_tokens[train:dev]
				origin_data[key]['resp'] = resp_tokens[train:dev]
				origin_data[key]['post_bert'] = post_bert_ids[train:dev]
				origin_data[key]['resp_bert'] = resp_bert_ids[train:dev]
			elif key == "test":
				origin_data[key] = {}
				origin_data[key]['post'] = post_tokens[dev:]
				origin_data[key]['resp'] = resp_tokens[dev:]
				origin_data[key]['post_bert'] = post_bert_ids[dev:]
				origin_data[key]['resp_bert'] = resp_bert_ids[dev:]
