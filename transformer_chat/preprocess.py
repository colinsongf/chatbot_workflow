...
def read_instances_from_file(sentences, max_sent_len):
    ...
    for i in tqdm(range(0, len(sentences) - 1, 2)):
        ...
        words = [w.word for w in tokenizer.segment(sentence_in)]
        if len(words) > max_sent_len:
            trimmed_sent_count += 1
        word_inst = words[:max_sent_len]
        if word_inst:
            word_src += [[Constants.BOS_WORD] + word_inst + [Constants.EOS_WORD]]
        else:
            word_src += [None]


        sentence_out = sentences[i + 1]
        words = [w.word for w in tokenizer.segment(sentence_out)]
        if len(words) > max_sent_len:
            trimmed_sent_count += 1
        word_inst = words[:max_sent_len]
        if word_inst:
            word_tgt += [[Constants.BOS_WORD] + word_inst + [Constants.EOS_WORD]]
        else:
            word_tgt += [None]
    ...
    word_insts = [word_src, word_tgt]

    return word_insts


def read_instances_from_file_src(inst_file, max_sent_len, keep_case):
    ''' Convert file into word seq lists and vocab '''
...
def main():
    ...
    parser.add_argument('-save_data', default=None)
    parser.add_argument('-max_len', '--max_word_seq_len', type=int, default=22)
    ...
    sentences = read_file(opt.train_src)
    print("processing training data")
    res = read_instances_from_file(sentences[:num_training_samples], opt.max_word_seq_len)

    train_src_word_insts = res[0]
    train_tgt_word_insts = res[1]
    ...
    valid_src_word_insts = res[0]
    valid_tgt_word_insts = res[1]
    '''
    # Validation set
